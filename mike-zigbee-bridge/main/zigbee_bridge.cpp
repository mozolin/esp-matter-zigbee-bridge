...
void zigbee_bridge_trigger_device_discovery(void)
{
    ...
    ESP_LOGW(TAG, "~~~ Discovery requests sent to all Zigbee devices");
    ...
    if (devices_found_count > 0) {
        ESP_LOGW(TAG, "~~~ Discovery completed: Found %d devices", devices_found_count);
        // Успешное завершение - быстро мигаем 5 раз
        //led_blink(5, 50);
    } else {
        ESP_LOGE(TAG, "~~~ Discovery completed: No devices found");
